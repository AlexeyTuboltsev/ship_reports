...
bool ParseObservations(const wxString &json, ObservationList &out,
  ...
  if (!root.HasMember(wxT("stations")) || !root[wxT("stations")].IsArray()) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    // --- Required fields ---
    if (!obj.HasMember(wxT("id")) || !obj[wxT("id")].IsString()) {
      skipped++;
      continue;
    }
    ...
    if (!obj.HasMember(wxT("lat")) || !obj[wxT("lat")].IsDouble()) {
      skipped++;
      continue;
    }
    ...
    if (!obj.HasMember(wxT("lon")) || !obj[wxT("lon")].IsDouble()) {
      skipped++;
      continue;
    }
    ...
    if (!obj.HasMember(wxT("time")) || !obj[wxT("time")].IsString()) {
      skipped++;
      continue;
    }
    wxDateTime obs_time;
...
#undef READ_OPT
  ...
  if (skipped > 0)
    wxLogWarning(
        "ShipObs: skipped %d station(s) with invalid/missing required fields",
        skipped);
