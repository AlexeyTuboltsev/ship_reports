 ...
 *
 * Equivalent to using Route & Mark Manager -> Layers -> "Create Persistent
 * Layer" and selecting the file. The layer appears on the chart immediately
 * without requiring an OpenCPN restart. The GPX file is copied to the
 * persistent layers directory so it reloads on next startup.
 *
...
/**
 * Returns the display names of all currently loaded layers.
 *
 * Layer names are the strings shown in the Route & Marks Manager layer list.
 * They are derived from the GPX filename stem at import time.  Plugin authors
 * can use this list to detect name collisions before calling
 * LoadGPXFileAsLayer().
 *
 * @return wxArrayString with one entry per loaded layer (order unspecified).
 */
extern DECL_EXP wxArrayString GetLayerNames();
