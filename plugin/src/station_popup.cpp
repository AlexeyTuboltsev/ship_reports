...
static int FindNearestStation(const ObservationList &stations,
                              ...
                              const wxPoint &cursor_px, wxWindow *parent,
...
bool HandleStationPopup(shipobs_pi *plugin, wxMouseEvent &event,
  ...
  if (want_dblclick && event.LeftDClick()) {
    if (!stations.empty()) {
      wxPoint st_screen;
      int idx = FindNearestStation(stations, vp, cursor_px, parent, &st_screen);
      if (idx >= 0) {
  ...
  if (want_hover && event.Moving()) {
    ...
    wxPoint st_screen;
    int best_idx =
        FindNearestStation(stations, vp, cursor_px, parent, &st_screen);
    if (best_idx >= 0) {
      if (!popup) popup = new StationPopup(parent);
      wxPoint screen_pos = parent->ClientToScreen(cursor_px);
      popup->ShowStation(stations[best_idx], screen_pos);
    } else {
      if (popup && popup->IsShown()) popup->Hide();
    }
