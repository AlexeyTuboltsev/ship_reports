...
int shipobs_pi::Init(void) {
  ...
  m_toolbar_id =
      InsertPlugInTool(_("Ship Obs"), &m_toolbar_bitmap, &m_toolbar_bitmap,
...
bool shipobs_pi::DeInit(void) {
  ...
  for (StationInfoFrame *f : m_info_frames) f->Destroy();
  m_info_frames.clear();
...
wxString shipobs_pi::GetShortDescription() {
  return _("Ship & Buoy Weather Observation Reports");
}
wxString shipobs_pi::GetLongDescription() {
  return _(
      "Displays real-time ship and buoy meteorological/oceanographic "
...
bool shipobs_pi::MouseEventHook(wxMouseEvent &event) {
  return HandleStationPopup(this, event, m_cursor_lat, m_cursor_lon, m_vp,
                            m_station_popup, m_parent_window);
}

static void RepositionInfoFrames(std::vector<StationInfoFrame *> &frames,
                                 PlugIn_ViewPort *vp, wxWindow *parent) {
...
static ObservationStation ParseStation(const wxJSONValue &s) {
  ...
  if (s.HasMember(wxT("country")))
    st.country = s.ItemAt(wxT("country")).AsString();
  ...
  if (s.HasMember(wxT("wind_dir")))
    st.wind_dir = SafeDouble(s.ItemAt(wxT("wind_dir")));
  if (s.HasMember(wxT("wind_spd")))
    st.wind_spd = SafeDouble(s.ItemAt(wxT("wind_spd")));
  if (s.HasMember(wxT("gust"))) st.gust = SafeDouble(s.ItemAt(wxT("gust")));
  if (s.HasMember(wxT("pressure")))
    st.pressure = SafeDouble(s.ItemAt(wxT("pressure")));
  if (s.HasMember(wxT("air_temp")))
    st.air_temp = SafeDouble(s.ItemAt(wxT("air_temp")));
  if (s.HasMember(wxT("sea_temp")))
    st.sea_temp = SafeDouble(s.ItemAt(wxT("sea_temp")));
  if (s.HasMember(wxT("wave_ht")))
    st.wave_ht = SafeDouble(s.ItemAt(wxT("wave_ht")));
...
static bool ReadHistoryFile(wxJSONValue &root) {
  ...
  wxString json = wxString::FromUTF8(static_cast<const char *>(buf.GetData()),
                                     buf.GetDataLen());
...
void shipobs_pi::LoadHistory() {
  ...
  for (int i = 0; i < records.Size(); i++) {
    ...
    FetchRecord rec;
    if (r.HasMember(wxT("label"))) rec.label = r[wxT("label")].AsString();
    if (r.HasMember(wxT("fetched_at"))) {
      wxDateTime dt;
      dt.ParseISOCombined(r[wxT("fetched_at")].AsString());
      if (dt.IsValid()) rec.fetched_at = dt;
    }
    if (r.HasMember(wxT("lat_min")))
      rec.lat_min = SafeDouble(r[wxT("lat_min")]);
    if (r.HasMember(wxT("lat_max")))
      rec.lat_max = SafeDouble(r[wxT("lat_max")]);
    if (r.HasMember(wxT("lon_min")))
      rec.lon_min = SafeDouble(r[wxT("lon_min")]);
    if (r.HasMember(wxT("lon_max")))
      rec.lon_max = SafeDouble(r[wxT("lon_max")]);
...
void shipobs_pi::AppendFetch(const FetchRecord &rec,
  ...
  if (!WriteHistoryFile(root))
    ...
    wxLogMessage("ShipObs: saved fetch record (%zu station(s))",
                 stations.size());
  LoadHistory();
}
...
void shipobs_pi::RemoveFetch(size_t index) {
  ...
  for (int i = 0; i < old_records.Size(); i++) {
    if (i != static_cast<int>(index)) new_records.Append(old_records[i]);
  }
  root[wxT("records")] = new_records;
