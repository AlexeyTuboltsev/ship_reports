...
StationInfoFrame::StationInfoFrame(wxWindow *parent, shipobs_pi *plugin,
    ...
    : wxFrame(parent, wxID_ANY, st.id, wxDefaultPosition, wxDefaultSize,
              wxFRAME_TOOL_WINDOW | wxFRAME_FLOAT_ON_PARENT | wxCAPTION |
  ...
  m_text = new wxStaticText(this, wxID_ANY, wxEmptyString, wxDefaultPosition,
  ...
  if (!std::isnan(st.wind_dir) || !std::isnan(st.wind_spd)) {
    wxString dir =
        std::isnan(st.wind_dir)
            ? wxString(_("--"))
            : wxString::Format(wxT("%d\u00b0T"), (int)std::round(st.wind_dir));
    wxString spd = std::isnan(st.wind_spd)
                       ? wxString(_("--"))
...
void StationInfoFrame::OnActivate(wxActivateEvent &event) {
  ...
  if (IsHighlighted() != was) RequestRefresh(m_plugin->GetParentWindow());
  event.Skip();
}
...
void StationInfoFrame::OnMove(wxMoveEvent &event) {
  if (!m_repositioning) m_offset = GetPosition() - m_last_station_px;
  event.Skip();
}
